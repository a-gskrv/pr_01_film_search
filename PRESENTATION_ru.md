...
___
Доступ к данным вынесен в отдельный слой repositories, а бизнес-логика сосредоточена в сервисах, что позволяет использовать одно и то же ядро с разными интерфейсами. Бизнес-логика не знает, как именно получаются данные — она работает только с методами репозитория.  Например, если завтра MySQL заменить на другой источник данных, изменения затронут только репозитории, а сервисы и интерфейсы останутся прежними.
